...
static void run_actions(int action_type)
{
	...
	for (a = init_action_list; a; a = a->next) {
#ifdef BIGAP
        if ((STATICACT & action_type) && action_type != a->action_type)
            continue;
        else
#endif
...
int init_main(int argc UNUSED_PARAM, char **argv)
{
	...
	while (1) {
		...
		while (1) {
			...
			if (a) {
#ifdef BIGAP
				printf("process '%s' (pid %d) exited. Scheduling for restart.\n",
						a->command, wpid);
#endif
				message(L_LOG, "process '%s' (pid %d) exited. Scheduling for restart.",
						a->command, wpid);
			}
