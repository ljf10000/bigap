...
{
    return block->size - (1+OS_CRC32_STRINGLEN) ;
}
...
{
    return block->cache + (1+OS_CRC32_STRINGLEN);
}
...
{
    char crcstring[1+OS_CRC32_STRINGLEN] = {0};
    unsigned long long crc = 0;
    os_memcpy(crcstring, block->cache, OS_CRC32_STRINGLEN);

    /*
    * uboot not support sscanf, so can not as below
    *   os_sscanf(crcstring, "%x", &crc);
    */
    crc = os_hex2number(crcstring, os_strlen(crcstring), 16, typeof(crc));
...
part_block_crc_write(struct part_block *block)
{
    ...
    os_memcpy(block->cache, crcstring, OS_CRC32_STRINGLEN);
    block->cache[OS_CRC32_STRINGLEN] = '\n';
}
...
part_line_read(struct part_block *block)
{
    ...
    while(NULL!=(begin = part_read_line(begin, end, line, PART_MAX_LINE)) || line[0]) {
        ...
        */
        ...
        os_str_reduce(line, is_separator);
...
*/
typedef mv_t part_var_foreach_f(struct part_block *block, struct string_kv *c, void *data);
...
part_var_foreach(struct part_block *block, part_var_foreach_f *foreach, void *data)
{
    ...
    list_for_each_entry_safe(item, n, &block->list, node) {
        mv.v = (*foreach)(block, &item->c, data);
        if (is_mv2_break(mv)) {
            return mv2_error(mv);
        }
    }
...
part_error(int err, char *name)
{
    switch(err) {
        case -ENOEXIST:
            os_println("%s not exist", os_safe_string(name));
            
            break;
        case -EEXIST:
            os_println("%s exist", os_safe_string(name));
...
rm_fille(char *filename)
{
    ...
    *   keep tmp file
    */
    if (false==__is_ak_debug_trace && filename[0]) {
