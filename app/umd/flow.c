...
__ip_handle(sock_server_t *server, bool first)
{
    ...
    if (__is_ak_debug_flow) {
        ...
        debug_flow("recv packet"
            ...
            dipstring,
            iph->ip_p,
            flow_dir_ops()->getname(flow.dir),
            flow_type_ops()->getname(flow.type),
            ipstring,
            macstring);
    }
...
um_jflow(void)
{
    ...
    for (i=0; i<um_pkt_type_end; i++) {
        ...
        for (j=0; j<um_pkt_check_end; j++) {
            ...
            jobj_add(jtype, pkt_check_ops()->getname(j), jst);
        }
        jobj_add(jtotal, pkt_check_ops()->getname(i), jtype);
    }
    ...
    for (i=0; i<um_flow_type_end; i++) {
        ...
        for (j=0; j<um_flow_dir_end; j++) {
            ...
            jobj_add(jtype, flow_dir_ops()->getname(j), jst);
        }
        jobj_add(jdev, flow_type_ops()->getname(i), jtype);
    }
