...
__set_reason(struct um_user *user, int reason)
{
    if (NULL==user) {
        ...
        debug_bug("same reason %s", deauth_reason_ops()->getname(reason));
    ...
    debug_trace("user(%s) state(%s) reason change %s==>%s", 
        ...
        user_state_ops()->getname(user->state),
        deauth_reason_ops()->getname(user->reason), 
        deauth_reason_ops()->getname(reason));
...
__set_state(struct um_user *user, int state)
{
    if (NULL==user) {
        ...
        debug_bug("same state %s", user_state_ops()->getname(state));
...
juser_flow(struct um_user *user, int type)
{
    ...
    for (dir=0; dir<um_flow_dir_end; dir++) {
        jobj_add(obj, flow_dir_ops()->getname(dir), __juser_flow(user, type, dir));
    }
...
juser_rate(struct um_user *user, int type)
{
    ...
    for (dir=0; dir<um_flow_dir_end; dir++) {
        jobj_add(obj, flow_dir_ops()->getname(dir), __juser_rate(user, type, dir));
    }
...
juser_limit(struct um_user *user)
{
    ...
    jobj_add(obj, flow_type_ops()->getname(um_flow_type_lan), __juser_limit(user, um_flow_type_lan));
    jobj_add(obj, flow_type_ops()->getname(um_flow_type_wan), __juser_limit(user, um_flow_type_wan));
...
jobj_t um_juser(struct um_user *user)
{
    ...
    jobj_add_string(obj, "state",   user_state_ops()->getname(user->state));
    jobj_add_string(obj, "reason",  deauth_reason_ops()->getname(user->reason));
...
touser_base(struct um_user *user, jobj_t juser)
{
    ...
    jj_byeq(user, juser, state, user_state_ops()->getid);
    jj_byeq(user, juser, reason, deauth_reason_ops()->getid);
...
touser_intf(struct um_limit *intf, jobj_t jintf)
{
    ...
    if (jobj) {
        for (dir=0; dir<um_flow_dir_end; dir++) {
            jobj_t jflow = jobj_get(jobj, flow_dir_ops()->getname(dir));
            if (jflow) {
    ...
    if (jobj) {
        for (dir=0; dir<um_flow_dir_end; dir++) {
            jobj_t jrate = jobj_get(jobj, flow_dir_ops()->getname(dir));
            if (jrate) {
...
touser_limit(struct um_user *user, jobj_t juser)
{
    ...
    if (jlimit) {
        for (type=0; type<um_flow_type_end; type++) {
            jobj_t jintf = jobj_get(jlimit, flow_type_ops()->getname(type));
            if (jintf) {
