...
init_env(void)
{
    ...
    env = env_gets(OS_ENVNAME(PWDFILE), NULL);
    if (env) {
        sys.env.pwdfile = env;
    }

    env = env_gets(OS_ENVNAME(VERSION), NULL);
    if (env) {
    ...
    env = env_gets(OS_ENVNAME(UPGRADE), NULL);
    if (env) {
        if (0==os_atoi(env)) {
    ...
    env = env_gets(OS_ENVNAME(ROOTFS), NULL);
    if (env) {
        int idx = os_atoi(env);
    ...
    env = env_gets(OS_ENVNAME(RESETBY), NULL);
    if (env) {
        int idx = os_atoi(env);
    ...
    env = env_gets(OS_ENVNAME(FORCE), NULL);
    if (env) {
        sys.env.force = env;
    }
    
    env = env_gets(OS_ENVNAME(SERVER), oem_rsync_server);
    if (env) {
        sys.env.server = env;
    }

    env = env_gets(OS_ENVNAME(PORT), oem_rsync_port);
    if (env) {
        if (0==os_atoi(env)) {
    ...
    env = env_gets(OS_ENVNAME(USER), oem_rsync_user);
    if (env) {
        sys.env.user = env;
    }

    env = env_gets(OS_ENVNAME(PATH), oem_rsync_path);
    if (env) {
        sys.env.path = env;
    }
