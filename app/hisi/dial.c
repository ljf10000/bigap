...
init_dial_env(void)
{
    dial.env.user = env_gets(OS_ENVNAME(USER), NULL);
    dial.env.password = env_gets(OS_ENVNAME(PASSWORD), NULL);
    dial.env.telephone = env_gets(OS_ENVNAME(TELEPHONE), NULL);
    dial.env.apn = env_gets(OS_ENVNAME(APN), NULL);
    
    dial.env.try = env_gets(OS_ENVNAME(TRY), NULL);
    if (dial.env.try) {
    ...
    dial.env.interval = env_gets(OS_ENVNAME(INTERVAL), NULL);
