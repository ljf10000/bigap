...
EXTERN void aes_ccm_prepare_first_ctr_blk(byte counter[], const byte nonce[], int nonce_len, int payload_len_store_size);
EXTERN void aes_ccm_prepare_first_format_blk(byte buf[], int assoc_len, int payload_len, int payload_len_store_size, int mac_len, const byte nonce[], int nonce_len);
EXTERN void aes_ccm_format_assoc_data(byte buf[], int *end_of_buf, const byte assoc[], int assoc_len);
EXTERN void aes_ccm_format_payload_data(byte buf[], int *end_of_buf, const byte payload[], int payload_len);
...
// table is only used by the (Inv)aesMixColumns steps.
...
DECLARE void 
aes_xor_buf(const byte in[], byte out[], size_t len)
{
	size_t idx;
...
*******************/
DECLARE int 
aes_encrypt_cbc(const byte in[], size_t in_len, byte out[], const uint32 key[], int keysize, const byte iv[])
{
	...
	for (idx = 0; idx < blocks; idx++) {
		...
		aes_xor_buf(iv_buf, buf_in, AES_BLOCK_SIZE);
...
DECLARE int 
aes_encrypt_cbc_mac(const byte in[], size_t in_len, byte out[], const uint32 key[], int keysize, const byte iv[])
{
	byte buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
DECLARE int 
aes_decrypt_cbc(const byte in[], size_t in_len, byte out[], const uint32 key[], int keysize, const byte iv[])
{
	byte buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
	...
	for (idx = 0; idx < blocks; idx++) {
		...
		aes_xor_buf(iv_buf, buf_out, AES_BLOCK_SIZE);
		memcpy(&out[idx * AES_BLOCK_SIZE], buf_out, AES_BLOCK_SIZE);
...
*******************/
DECLARE void 
aes_increment_iv(byte iv[], int counter_size)
{
	int idx;
...
DECLARE void 
aes_encrypt_ctr(const byte in[], size_t in_len, byte out[], const uint32 key[], int keysize, const byte iv[])
{
	...
	if (in_len > AES_BLOCK_SIZE) {
		for (idx = 0; idx < last_block_length; idx += AES_BLOCK_SIZE) {
			...
			aes_xor_buf(out_buf, &out[idx], AES_BLOCK_SIZE);
			aes_increment_iv(iv_buf, AES_BLOCK_SIZE);
		}
	}

	aes_encrypt(iv_buf, out_buf, key, keysize);
	aes_xor_buf(out_buf, &out[idx], in_len - idx);   // Use the Most Significant bytes.
}

DECLARE void 
aes_decrypt_ctr(const byte in[], size_t in_len, byte out[], const uint32 key[], int keysize, const byte iv[])
{
...
DECLARE int
aes_encrypt_ccm(const byte payload[], uint32 payload_len, const byte assoc[], unsigned short assoc_len,
                    const byte nonce[], unsigned short nonce_len, byte out[], uint32 *out_len,
                    uint32 mac_len, const byte key_str[], int keysize)
{
	...
	aes_ccm_prepare_first_format_blk(buf, assoc_len, payload_len, payload_len_store_size, mac_len, nonce, nonce_len);
	end_of_buf = AES_BLOCK_SIZE;

	// Format the Associated Data, aka, assoc[].
	aes_ccm_format_assoc_data(buf, &end_of_buf, assoc, assoc_len);

	// Format the Payload, aka payload[].
	aes_ccm_format_payload_data(buf, &end_of_buf, payload, payload_len);

	// Create the first counter block.
	aes_ccm_prepare_first_ctr_blk(counter, nonce, nonce_len, payload_len_store_size);
	...
	aes_increment_iv(temp_iv, AES_BLOCK_SIZE - 1 - mac_len);   // Last argument is the byte size of the counting portion of the counter block. /*BUG?*/
...
DECLARE int 
aes_decrypt_ccm(const byte ciphertext[], uint32 ciphertext_len, const byte assoc[], unsigned short assoc_len,
                    const byte nonce[], unsigned short nonce_len, byte plaintext[], uint32 *plaintext_len,
                    uint32 mac_len, int *mac_auth, const byte key_str[], int keysize)
{
	...
	*plaintext_len = ciphertext_len - mac_len;
	...
	aes_ccm_prepare_first_ctr_blk(counter, nonce, nonce_len, plaintext_len_store_size);

	// Decrypt the Payload with CTR mode with a counter starting at 1.
	memcpy(temp_iv, counter, AES_BLOCK_SIZE);
	aes_increment_iv(temp_iv, AES_BLOCK_SIZE - 1 - mac_len);   // (AES_BLOCK_SIZE - 1 - mac_len) is the byte size of the counting portion of the counter block.
	...
	if (mac_auth != NULL) {
		...
		// Format the first block of the formatted data.
		plaintext_len_store_size = AES_BLOCK_SIZE - 1 - nonce_len;
		aes_ccm_prepare_first_format_blk(buf, assoc_len, *plaintext_len, plaintext_len_store_size, mac_len, nonce, nonce_len);
		end_of_buf = AES_BLOCK_SIZE;
		...
		aes_ccm_format_payload_data(buf, &end_of_buf, plaintext, *plaintext_len);
...
DECLARE void 
aes_ccm_prepare_first_ctr_blk(byte counter[], const byte nonce[], int nonce_len, int payload_len_store_size)
{
...
DECLARE void 
aes_ccm_prepare_first_format_blk(byte buf[], int assoc_len, int payload_len, int payload_len_store_size, int mac_len, const byte nonce[], int nonce_len)
{
...
DECLARE void 
aes_ccm_format_assoc_data(byte buf[], int *end_of_buf, const byte assoc[], int assoc_len)
{
	int pad;
...
DECLARE void 
aes_ccm_format_payload_data(byte buf[], int *end_of_buf, const byte payload[], int payload_len)
{
	int pad;
...
DECLARE uint32 
aesSubWord(uint32 word)
{
    static const byte sbox[16][16] = AES_SBOX_INITER;
...
DECLARE void 
aes_key_setup(const byte key[], uint32 w[], int keysize)
{
	int Nb=4,Nr,Nk,idx;
	uint32 temp, Rcon[]={0x01000000,0x02000000,0x04000000,0x08000000,0x10000000,0x20000000,
	...
	for (idx = Nk; idx < Nb * (Nr+1); ++idx) {
		temp = w[idx - 1];
		if ((idx % Nk) == 0)
			temp = aesSubWord(AES_KE_ROTWORD(temp)) ^ Rcon[(idx-1)/Nk];
...
// Performs the aesAddRoundKey step. Each round has its own pre-generated 16-byte key in the
...
DECLARE void 
aesAddRoundKey(byte state[][4], const uint32 w[])
{
	byte subkey[4];
...
// Performs the aesSubBytes step. All bytes in the state are substituted with a
// pre-calculated value from a lookup table.
DECLARE void 
aesSubBytes(byte state[][4])
{
    static const byte sbox[16][16] = AES_SBOX_INITER;
...
DECLARE void 
aesInvSubBytes(byte state[][4])
{
    static const byte invsbox[16][16] = AES_INVSBOX_INITER;
...
// Performs the aesShiftRows step. All rows are shifted cylindrically to the left.
DECLARE void 
aesShiftRows(byte state[][4])
{
	int t;
...
DECLARE void 
aesMixColumns(byte state[][4])
{
    static const byte gf_mul[256][6] = AES_GF_MUL_INITER;
...
DECLARE void aesInvMixColumns(byte state[][4])
{
    static const byte gf_mul[256][6] = AES_GF_MUL_INITER;
...
DECLARE void 
aes_encrypt(const byte in[], byte out[], const uint32 key[], int keysize)
{
	byte state[4][4];
	...
	// The last round does not perform the aesMixColumns step.
	aesAddRoundKey(state,&key[0]);
	aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[4]);
	aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[8]);
	aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[12]);
	aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[16]);
	aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[20]);
	aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[24]);
	aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[28]);
	aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[32]);
	aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[36]);
	if (keysize != 128) {
		aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[40]);
		aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[44]);
		if (keysize != 192) {
			aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[48]);
			aesSubBytes(state); aesShiftRows(state); aesMixColumns(state); aesAddRoundKey(state,&key[52]);
			aesSubBytes(state); aesShiftRows(state); aesAddRoundKey(state,&key[56]);
		}
		else {
			aesSubBytes(state); aesShiftRows(state); aesAddRoundKey(state,&key[48]);
		}
	}
	else {
		aesSubBytes(state); aesShiftRows(state); aesAddRoundKey(state,&key[40]);
	}
...
DECLARE void 
aes_decrypt(const byte in[], byte out[], const uint32 key[], int keysize)
{
	byte state[4][4];
	...
	if (keysize > 128) {
		if (keysize > 192) {
			...
			aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[52]);aesInvMixColumns(state);
			aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[48]);aesInvMixColumns(state);
		}
		else {
			aesAddRoundKey(state,&key[48]);
		}
		aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[44]);aesInvMixColumns(state);
		aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[40]);aesInvMixColumns(state);
	}
	else {
		aesAddRoundKey(state,&key[40]);
	}
	aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[36]);aesInvMixColumns(state);
	aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[32]);aesInvMixColumns(state);
	aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[28]);aesInvMixColumns(state);
	aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[24]);aesInvMixColumns(state);
	aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[20]);aesInvMixColumns(state);
	aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[16]);aesInvMixColumns(state);
	aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[12]);aesInvMixColumns(state);
	aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[8]);aesInvMixColumns(state);
	aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[4]);aesInvMixColumns(state);
	aesInvShiftRows(state);aesInvSubBytes(state);aesAddRoundKey(state,&key[0]);
