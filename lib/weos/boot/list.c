...
DECLARE void list_del_init(struct list_head *entry)
{
	__list_del_entry(entry);
	INIT_LIST_HEAD(entry);
}

DECLARE void list_move(struct list_head *list, struct list_head *head)
{
	__list_del_entry(list);
	list_add(list, head);
}

DECLARE void list_move_tail(struct list_head *list,
...
DECLARE void list_cut_position(struct list_head *list,
		struct list_head *head, struct list_head *entry)
{
	if (list_empty(head))
		return;
...
DECLARE void list_splice_tail(struct list_head *list,
				struct list_head *head)
{
	if (!list_empty(list))
...
DECLARE void list_splice_init(struct list_head *list,
				    struct list_head *head)
{
	if (!list_empty(list)) {
		__list_splice(list, head, head->next);
...
DECLARE void list_splice_tail_init(struct list_head *list,
					 struct list_head *head)
{
	if (!list_empty(list)) {
		__list_splice(list, head->prev, head);
...
DECLARE void hlist_add_head(struct hlist_node *n, struct hlist_head *h)
{
...
DECLARE void hlist_add_before(struct hlist_node *n,
...
DECLARE void hlist_add_after(struct hlist_node *n,
					struct hlist_node *next)
{
	next->next = n->next;
	n->next = next;
...
DECLARE void hlist_move_list(struct hlist_head *old,
