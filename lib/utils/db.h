...
{                                       \
    int i, err, count = os_count_of(table->hash); \
                                        \
    for (i=0; i<count; i++) {           \
        err = hash_add(&table->hash[i], &node->hash[i], ncalc[i]); \
        if (err<0) {                    \
...
#define DECLARE_DB_HX(_number, _table, _mod, _type, _member) \
static inline _type *                                   \
...
{                                                       \
    return hx_entry(node, _type, _member, nidx);        \
                                                        ...
                                                        \
static inline _type *                                   \
_mod##_h##_number##_entry(h##_number##_node_t *node)    \
{                                                       \
    return h##_number##_entry(node, _type, _member);    \
}                                                       \
                                                        \
static inline int                                       \
_mod##_foreach(mv_t (*foreach)(_type *entry), bool safe) \
{                                                       \
    mv_t node_foreach(h##_number##_node_t *node)        \
    {                                                   \
        return (*foreach)(_mod##_h##_number##_entry(node)); \
    }                                                   \
                                                        \
    return __h##_number##_foreach(_table, node_foreach, safe); \
}                                                       \
                                                        \
os_fake_declare                                         \
/* end */
...
* int __h1_add(__h1_table_t *table, __h1_node_t *node, hash_node_calc_f *ncalc);
* void __h1_del(__h1_table_t *table, __h1_node_t *node);
* __h1_node_t *__h1_find(__h1_table_t *table, hash_data_calc_f *dcalc, hash_eq_f *eq);
...
static inline int
h1_add(h1_table_t *table, h1_node_t *node, hash_node_calc_f *ncalc)
{
    hash_node_calc_f *hash[] = {ncalc};
...
h1_find(h1_table_t *table, hash_data_calc_f *dcalc, hash_eq_f *eq)
{
    return __h1_find(table, 0, dcalc, eq);
}
    ...
    {                                                       \
        ...
        {                                                   \
            return (*foreach)(_mod##_h1_entry(node));       \
                                                            ...
                                                            \
        return __h1_foreach(_table, node_foreach, safe);    \
    }                                                       \
                                                            \
    os_fake_declare                                         \
    /* end */
...
* __hx_node_t *__hx_add(__hx_table_t *table, __hx_node_t *node, hash_node_calc_f *ncalc[]);
* void __hx_del(__hx_table_t *table, __hx_node_t *node);
* __hx_node_t *__hx_find(__hx_table_t *table, int hidx, hash_data_calc_f *dcalc, hash_eq_f *eq);
...
static inline int
h2_add(h2_table_t *table, h2_node_t *node, hash_node_calc_f *ncalc[])
{
    return __h2_add(table, node, ncalc);
}
...
h2_find(h2_table_t *table, int hidx, hash_data_calc_f *dcalc, hash_eq_f *eq)
{
    return __h2_find(table, hidx, dcalc, eq);
}
...
static inline int
h3_add(h3_table_t *table, h3_node_t *node, hash_node_calc_f *ncalc[])
{
    return __h3_add(table, node, ncalc);
}
...
h3_find(h3_table_t *table, int hidx, hash_data_calc_f *dcalc, hash_eq_f *eq)
{
    return __h3_find(table, hidx, dcalc, eq);
}
...
static inline int
h4_add(h4_table_t *table, h4_node_t *node, hash_node_calc_f *ncalc[])
{
    return __h4_add(table, node, ncalc);
}
...
h4_find(h4_table_t *table, int hidx, hash_data_calc_f *dcalc, hash_eq_f *eq)
{
    return __h4_find(table, hidx, dcalc, eq);
}
...
static inline int
h5_add(h5_table_t *table, h5_node_t *node, hash_node_calc_f *ncalc[])
{
    return __h5_add(table, node, ncalc);
}
...
h5_find(h5_table_t *table, int hidx, hash_data_calc_f *dcalc, hash_eq_f *eq)
{
    return __h5_find(table, hidx, dcalc, eq);
}
...
static inline int
h6_add(h6_table_t *table, h6_node_t *node, hash_node_calc_f *ncalc[])
{
    return __h6_add(table, node, ncalc);
}
...
h6_find(h6_table_t *table, int hidx, hash_data_calc_f *dcalc, hash_eq_f *eq)
{
    return __h6_find(table, hidx, dcalc, eq);
}
...
static inline int
h7_add(h7_table_t *table, h7_node_t *node, hash_node_calc_f *ncalc[])
{
    return __h7_add(table, node, ncalc);
}
...
h7_find(h7_table_t *table, int hidx, hash_data_calc_f *dcalc, hash_eq_f *eq)
{
    return __h7_find(table, hidx, dcalc, eq);
}
...
static inline int
h8_add(h8_table_t *table, h8_node_t *node, hash_node_calc_f *ncalc[])
{
    return __h8_add(table, node, ncalc);
}
...
h8_find(h8_table_t *table, int hidx, hash_data_calc_f *dcalc, hash_eq_f *eq)
{
    return __h8_find(table, hidx, dcalc, eq);
}
...
static inline int
h9_add(h9_table_t *table, h9_node_t *node, hash_node_calc_f *ncalc[])
{
    return __h9_add(table, node, ncalc);
}
...
h9_find(h9_table_t *table, int hidx, hash_data_calc_f *dcalc, hash_eq_f *eq)
{
    return __h9_find(table, hidx, dcalc, eq);
}
