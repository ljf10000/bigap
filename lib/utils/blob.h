...
{
    return blob_value(blob) + blob_value_len(blob);
}
...
{
	return *blob_pointer(uint32_t, blob);
}

static inline uint64_t
blob_get_u64(const blob_t *blob)
{
	return *blob_pointer(uint64_t, blob);
}

static inline int32_t
blob_get_i32(const blob_t *blob)
{
    return *blob_pointer(int32_t, blob);
}

static inline int64_t
blob_get_i64(const blob_t *blob)
{
	return *blob_pointer(int64_t, blob);
}
...
#define blob_for_each_attr(_pos, _blob, _rem)           \
	for (_rem = _blob ? blob_size(_blob) : 0,           \
	        _pos = _blob?blob_pointer(blob_t, _blob):0;   \
	     _rem > 0 && (blob_size(_pos) <= _rem) &&       \
	        (blob_size(_pos) >= sizeof(blob_t));        \
	     _rem -= blob_size(_pos), _pos = blob_next(_pos)) \
    /* end */
...
static inline bool
blob_base_check(const void *value, uint32_t len, int type)
{
    static struct {
        uint32_t flag;
        uint32_t minsize;
    ...
    blob_for_each_attr(pos, blob, rem) {
        ...
        */
        ...
        if (rule) {
            ...
            if (false==blob_base_check(blob_value(pos), len, type)) {
                continue;
            }
...
static inline blob_t *
blob_put_binary(slice_t *slice, int id, const char *name, const void *binary, uint32_t len)
{
...
__blob_byteorder(blob_t *blob, bool ntoh)
{
    ...
    switch(type) {
        ...
        case BLOB_T_INT64:           
            *blob_pointer(int64_t, blob) = bswap_64(blob_get_u64(blob));
            break;
        case BLOB_T_BOOL:
        case BLOB_T_INT32:
            *blob_pointer(int32_t, blob) = bswap_32(blob_get_u32(blob));
            break;
        case BLOB_T_STRING:
        case BLOB_T_BINARY:
        case BLOB_T_EMPTY:
